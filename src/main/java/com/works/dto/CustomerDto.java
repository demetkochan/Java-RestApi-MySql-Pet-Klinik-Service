...
public class CustomerDto {
    ...
    public Map<String, Object> search( String data, String pageNumber ) {
        Map<String, Object> hm = new LinkedHashMap<>();
        int ipageNumber = Integer.parseInt(pageNumber);
        Pageable pageable = PageRequest.of(ipageNumber, 10);
        List<Customer> ulist = cRepo.findByCnameContainsOrCsurnameContainsAllIgnoreCaseOrderByCidAsc(data,data,pageable);
        long totalSize = cRepo.countByCnameContainsOrCsurnameContainsAllIgnoreCase(data,data);
        if ( ulist.size() > 0 ) {
            hm.put("status", true);
            hm.put("message", "Arama işlemi başarılı!");
            hm.put("totalSize", totalSize );
            hm.put("result", ulist);
        }else {
            hm.put("status", false);
            hm.put("message", "Arama işlemini farklı bir kelime ile yapınız, Sonuç yok!");
            hm.put("result", ulist);
        }
        return hm;
    }
